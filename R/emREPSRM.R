#' @export
emREPSRM = function(X, Y, splineOrder, splineType = c("spline", "B-spline"), nknots, verbose = TRUE) {
  ...
  knots <- seq.int(from = 0, to = 1, length.out = nknots + 2) # including the first and the two boudaries knots
  # knots(end-1) = x(end)+x(end)-x(end-1)+x(end)-x(end-1);
  knots[length(knots)] <- X[m] + X[m] - X[m - 1]
  ...
  if (splineType == "spline") {
    knots <- knots[2:(length(knots) - 1)] # Take the interior knots
    X <- splinebasis(X, knots, splineOrder)
  } else if (splineType == "B-spline") {
    X <- bsplinebasis(X, knots, splineOrder)
  }
  ...
  while(iter <= MaxIter && !converged) {
    ...
    if (length(small_klas) > 0) {# Discard the small clusters
